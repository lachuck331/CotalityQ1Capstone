...
# Path to the California State shapefile (used as the clipping mask)
CA_STATE_PATH = DATA_DIR / "ca_state" / "ca_state.shp"

# Output directory for the processed file
OUTPUT_DIR = DATA_DIR / "ca_mbts"
...
def filter_state(input_path: Path, ca_state: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    Load the MTBS perimeter file and clip it to California State.
    """
    ...
    if mtbs_data.crs != ca_state.crs:
        print("CRS mismatch. Reprojecting MTBS data to match CA State...")
        mtbs_data = mtbs_data.to_crs(ca_state.crs)
    else:
        print("CRS matches. No reprojection needed.")

    # Clip to CA State
    print("Clipping MTBS data to California State...")
    mtbs_clipped = gpd.clip(mtbs_data, ca_state)
    
    if mtbs_clipped.empty:
        print("WARNING: The clipped dataset is empty! No fires found within the CA State boundary.")
    else:
        print(f"Clipping complete. Found {len(mtbs_clipped)} fires within California State.")
...
def verify_state(gdf: gpd.GeoDataFrame, ca_state: gpd.GeoDataFrame):
    """
    Verify the clipped data overlaps with California State.
    """
    print("\nVerifying spatial overlap with CA State...")
    ...
    # Check CRS Match
    if gdf.crs != ca_state.crs:
        print(f"FAILED: CRS mismatch. Output: {gdf.crs}, Reference: {ca_state.crs}")
        return

    # Check Bounds
    out_bounds = gdf.total_bounds
    state_bounds = ca_state.total_bounds
    
    # Check for no overlap (disjoint)
    is_disjoint = (out_bounds[0] > state_bounds[2] or  # out_minx > state_maxx
                   out_bounds[2] < state_bounds[0] or  # out_maxx < state_minx
                   out_bounds[1] > state_bounds[3] or  # out_miny > state_maxy
                   out_bounds[3] < state_bounds[1])    # out_maxy < state_miny
                   
    if is_disjoint:
         print("FAILED: Output bounds do not overlap with California State.")
         return
         
    print(f"SUCCESS: Spatial verification passed. {len(gdf)} features overlap with CA State.")
...
def main():
    ...
    if not CA_STATE_PATH.exists():
        print(f"ERROR: CA State file not found at {CA_STATE_PATH}")
        return
    
    # 3. Load Data
    # Read the California State shapefile
    ca_state = gpd.read_file(CA_STATE_PATH)
    print(f"Loaded CA State boundary. CRS: {ca_state.crs}")

    # 4. Clip to State
    clipped_gdf = filter_state(INPUT_MTBS_PATH, ca_state)
